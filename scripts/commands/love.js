...
module.exports.onLoad = async () => {
  const dir = __dirname + "/cache/Picsart_25-07-07_12-54-20-382.png";
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  const bgPath = path.join(dir, "template.png");
  if (!fs.existsSync(bgPath)) {
    await global.utils.downloadFile("https://i.imgur.com/5uUfYUj.jpeg", bgPath); // Custom love bg
  }
};
...
module.exports.run = async function ({ event, api }) {
  ...
  const loveDir = path.join(__dirname, "cache", "Picsart_25-07-07_12-54-20-382.png");
  ...
  const getAvt = async (id, filePath) => {
    const res = await axios.get(`https://graph.facebook.com/${id}/picture?width=512&height=512&access_token=6628568379|c1e620fa708a1d5696fb991c1bde5662`, {
